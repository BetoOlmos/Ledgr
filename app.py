...
def run_live_pulse_library(current, prior):
    ...
    if c_ratio >= 1.5:
        r1_ans = (
            f"Yes. You hold ${c:,.0f} in capital against "
            f"${liab:,.0f} in liabilities. You have "
            f"${c_ratio:.2f} for every $1.00 of bills."
        )
        if c_ratio > p_ratio and p_ratio > 0:
            r1_class = "ratio-why"
            r1_why = (
                f"Why it changed: Your cushion improved from "
                f"${p_ratio:.2f} last period because you "
                f"reduced liabilities by ${pliab - liab:,.0f}."
            )
        else:
            r1_why = (
                "Why it changed: Your bill-paying balance "
                "remains steady. Cash keeps pace with liabilities."
            )
    else:
        r1_ans = (
            f"No, tight. You have ${c_ratio:.2f} for every "
            f"$1.00 of upcoming bills. Total debt is "
            f"${liab:,.0f} against ${c:,.0f} in cash."
        )
        r1_class = "ratio-why-down"
        if c_ratio < p_ratio:
            r1_why = (
                f"Why it changed: Dropped from ${p_ratio:.2f}. "
                f"Your accounts payable grew by ${liab - pliab:,.0f}, "
                f"outpacing your available cash reserves."
            )
        else:
            r1_why = (
                "Why it changed: Working capital ratio is restricted. "
                "Immediate revenue is needed to clear outstanding debt."
            )
    ...
    if ar > 0:
        r2_ans = (
            f"Your cash is trapped. You hold ${ar:,.0f} "
            f"in unpaid customer invoices that have passed "
            f"standard settlement timelines."
        )
        if ar > par and par > 0:
            r2_class = "ratio-why-down"
            r2_why = (
                f"Why it changed: Invoices grew by ${ar - par:,.0f} "
                f"compared to last period. This delayed cash collection "
                f"is making the bank account feel emptier."
            )
        else:
            r2_class = "ratio-why"
            r2_why = (
                f"Why it changed: Collection habits are improving. "
                f"You clawed back ${par - ar:,.0f} from balances, "
                f"turning invoices into real bank checking deposits."
            )
    else:
        r2_ans = (
            "Your invoicing pipelines are completely clear. "
            "All sales have been converted into liquid cash."
        )
        r2_why = (
            "Why it changed: Upfront payment structures keep "
            "your operational cash cycle perfectly clean."
        )
    ...
    if margin >= 15:
        r3_ans = (
            f"Yes, highly efficient. You captured a "
            f"{margin:.1f}% net margin, turning sales into "
            f"${p:,.0f} of real take-home wealth."
        )
        if margin > p_margin and p_margin > 0:
            r3_class = "ratio-why"
            r3_why = (
                f"Why it changed: Expanded from {p_margin:.1f}% "
                f"last period. Efficiency grew because you raised "
                f"revenue without increasing your fixed overhead costs."
            )
        else:
            r3_why = (
                "Why it changed: Your bottom-line capture remains "
                "solid, maintaining steady returns across cycles."
            )
    else:
        r3_ans = (
            f"No, growth is inefficient. Net margin fell "
            f"to {margin:.1f}%, leaving you with ${p:,.0f} "
            f"in profit from a large ${r:,.0f} sales volume."
        )
        r3_class = "ratio-why-down"
        if margin < p_margin:
            r3_why = (
                f"Why it changed: Fell from {p_margin:.1f}% last period. "
                f"Operating expenses increased by ${e - pe:,.0f}, "
                f"directly cutting into your revenue gains."
            )
        else:
            r3_why = (
                "Why it changed: Conversion efficiency remains low. "
                "Fulfillment costs are taking too big a bite out of sales."
            )
...
rev_time = (
parse_accounting_csv(
pl_file,
["total revenue", "total income", "gross sales"],
)
or {}
)
exp_time = (
parse_accounting_csv(
pl_file,
["total expense", "total operating expenses"],
)
or {}
)
prof_time = (
...
st.warning("⚠️ Running demo playground data.")
